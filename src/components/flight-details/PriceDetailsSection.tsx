...
import { AirItineraryPricingInfo } from "../../types/sabreReturnTypes";
interface PriceDetailsProps {
    data: Partial<AirItineraryPricingInfo>
}
const PriceDetailsSection = ({ data }: PriceDetailsProps): JSX.Element => {
    ...
    return (
        <>
            ...
            {data && data?.ItinTotalFare?.BaseFare.CurrencyCode && (
                <div className="col-md-4 col-lg-3">
                    <div className="make_box123">
                        ...
                        <div className="details_usd">
                            <h4>Price Details ({data?.ItinTotalFare?.BaseFare.CurrencyCode || "USD"})</h4>
                            <table className="usd_list">
                                ...
                                <tbody>
                                    <tr>
                                        <td>Base Fare</td>
                                        <td>
                                            <p>{data?.ItinTotalFare?.BaseFare.CurrencyCode || "USD"}{" "}{String(data?.ItinTotalFare?.BaseFare?.Amount.toFixed(2)).split(".")[0]}<em>.{
                                                String(data?.ItinTotalFare?.BaseFare?.Amount.toFixed(2)).split(".")[1]
                                            }</em></p>
                                        </td>
                                    </tr>
                                    <tr>
                                        <td>Tax & Charges</td>
                                        <td>
                                            <p>{data?.ItinTotalFare?.Taxes.Tax[0].CurrencyCode}{" "}{String(data?.ItinTotalFare?.Taxes.Tax[0].Amount.toFixed(2)).split(".")[0]} <em>.{String(data?.ItinTotalFare?.Taxes.Tax[0].Amount.toFixed(2)).split(".")[1]}</em></p>
                                        </td>
                                    </tr>
                                    {/* <tr>
                     ...
                     <td>
                         <p>USD 1920<em>.08</em></p>
                            ...
                            <table className="usd_list">
                                <thead>
                                    <tr>
                                        <th>Total Price ({data?.ItinTotalFare?.TotalFare.CurrencyCode || "USD"})</th>
                                        <th>{data?.ItinTotalFare?.TotalFare.CurrencyCode}{" "}{data?.ItinTotalFare?.TotalFare.Amount.toFixed(2)}</th>
                                    </tr>
                                </thead>
                            </table>
